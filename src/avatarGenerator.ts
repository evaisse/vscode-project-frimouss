...
export class AvatarGenerator {
    ...
    /**
     * Get a color from the current VSCode theme
     */
    private static getThemeColors(): AvatarConfig {
     ...
     */
    private static generateColorFromHash(hash: number, saturation: number = 70, lightness: number = 50): string {
        const hue = hash % 360;
     ...
     */
    private static getCharactersFromName(name: string): string[] {
    ...
    public static generateAvatarDataUri(projectName: string): string {
        const svg = this.generateAvatar(projectName);
